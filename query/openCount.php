...
// Check if the 'type-of-delivery' parameter is set
    $Delivery_types = ['Laptops','Monitors','Desktops','Printers'];
    ...
    $data = [];
    $columns = [];  // This will store model names (or "located" values)
    ...
    $stmtDesk->execute();
    $models = $stmtDesk->fetchAll(PDO::FETCH_ASSOC);
    ...
    foreach ($models as $row) {
        ...
        $searchSQL = "SELECT COUNT(*) AS count , EQ_Model AS Model, EQ_type AS Hardware FROM OpenEquipment WHERE EQ_Model LIKE :model";
        $stmtD = $pdo->prepare($searchSQL);
        $stmtD->bindParam(':model', $model, PDO::PARAM_STR);  // Bind the model value
        $stmtD->execute();
        $countRow = $stmtD->fetch(PDO::FETCH_ASSOC);


        $finalData[] = [
            'Quantity' => $countRow['count'],
            'Model' => $model,
